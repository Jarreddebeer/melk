...
- A back-edge wraps around the diagram's perimeter, so it does not take a forward face off the source's flow axis. Instead both endpoints attach to the face *perpendicular* to flow (N/S under LR, E/W under TB), choosing whichever margin is nearer the two endpoints; the trace wraps over that edge. See §6.4 and `backEdgeWrapSide` in `slots.ts`.

Because anchoring forms a DAG (each node is placed by exactly one primitive), the propagation always terminates.

**Storage.** The placer caches forward per node in `Placement.forwardAt: Map<NodeId, Direction>`, populated during the anchor pass. Subsequent stages query the cache; they do not re-derive forward from layout mode or cell deltas. Edges carry their *own* forward — for a forward edge, this is `direction(src.cell → tgt.cell)` after placement. A back-edge doesn't derive faces from a forward at all: it attaches both endpoints to the perpendicular-to-flow margin face (`backEdgeWrapSide`, §6.4) and the perimeter router wraps it around the diagram.
...
Declares an explicit back-edge. It attaches to the perpendicular-to-flow margin face and the router wraps it around the diagram's perimeter (§2.5).
...
- **Back-edges wrap the perimeter.** *(Updated 2026-06-21; originally "back-edges take the rear face of source and target and may use diagonals like any corridor".)* Back-edges now attach to the perpendicular-to-flow margin face (§2.5) and the perimeter router wraps them around the outside of the diagram as an orthogonal C/n shape — two stub legs along the flow axis plus one run along a clear margin row/col. See `backEdgeWrapSide` (`slots.ts`) and the perimeter dispatch (`channels.ts`).
...
- **Back-edge faces = perpendicular to flow, nearer margin.** *(Updated 2026-06-21; the original rule below took the rear face — `opposite(source's local forward)` — which under LR exited the source's W face and entered the target's E face, attaching the wrap to the side edges. That read wrong: the trace wraps over the top/bottom but clung to a box corner. The current rule attaches both endpoints to the perpendicular-to-flow face — N/S under LR, E/W under TB — centred on whichever margin is nearer the two endpoints, so the trace leaves and re-enters on the face it actually wraps toward. See `backEdgeWrapSide` in `slots.ts` and the perimeter dispatch in `channels.ts`.)*
...
For back-edges, `pivot:` is inert. The "above-bounding-box vs below-bounding-box" wrap choice it once gestured at is now resolved by `backEdgeWrapSide` (§2.5) — it picks the nearer margin automatically, and `exit:`/`entry:` (§6.4) override it explicitly — so `pivot:` has nothing left to control here. Authoring a `pivot:` on a back-edge is *accepted but inert* rather than an error, keeping older files valid.
...
- **Back-edge default differs.** A back-edge ignores `edgeFwd` for side assignment: both sides default to the perpendicular-to-flow margin face from `backEdgeWrapSide` (§2.5). `exit:`/`entry:` override either side exactly as above, letting an author flip an LR back-edge from over-the-top (N/N) to under-the-bottom (S/S).
...
- **Back-edges accept `exit:` / `entry:`** *(updated 2026-06-21)*. The original design deferred them with an `E_EXIT_ON_BACK_EDGE` error, but back-edges now have a well-defined default face (the perpendicular-to-flow margin, §2.5/§6.4), so the overrides plug in cleanly: they flip which margin the back-edge wraps over. No error is raised. The code never shipped `E_EXIT_ON_BACK_EDGE`; only via-edges reject the keys (`E_EXIT_ON_VIA_EDGE`).
